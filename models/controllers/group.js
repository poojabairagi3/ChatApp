...
exports.postgroup=async(req,res,next)=>{
    try{
        ...
        console.log(user);
        console.log(group);
        res.status(201).json({massage: 'group added successfully', gp,success:true})
    }
    catch(err){
        console.log(err);
    }
...
exports.postmsgs=async(req,res,next)=>{
    try{
       ...
       groupmessage.userId=req.user.id;
        ...
        console.log(gm);
        res.status(201).json({massage: 'msg send successfully', gm,success:true})
    }
    catch(err){
        console.log(err);
    }
...
exports.getgroupmsgs=async(req,res,next)=>{
    try{
        ...
        res.status(200).json({message:'groupmsg get successfully',groupmsg,success:true})
    }
    catch(err){
        console.log(err);
    }
